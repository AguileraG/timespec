...
struct timespec timespec_mod(struct timespec ts1, struct timespec ts2)
{
     ...
     */
    while ((ts2.tv_sec < (INT_LEAST64_MAX >> 1)) && timespec_ge(ts1, ts2)) {
        i++;
        ts2.tv_nsec <<= 1;
        ts2.tv_sec <<= 1;
...
/** \fn struct timespec timespec_clamp(struct timespec ts, struct timespec min, struct
 * timespec max) \brief Clamp the value of TS between MIN and MAX.
 */
struct timespec timespec_clamp(struct timespec ts, struct timespec min,
                               struct timespec max)
{
...
struct timespec timespec_from_double(double s)
{
    struct timespec ts = {
        .tv_sec = s,
        .tv_nsec = (s - (int_least64_t)(s)) * NSEC_PER_SEC,
    };
...
/** \fn struct timespec timespec_from_ms(int_least64_t milliseconds)
...
/** \fn int_least64_t timespec_to_ms(struct timespec ts)
    ...
    {                                                                                   \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {                   \
            printf("%s:%d: timespec_normalise({%ld, %ld}) returned wrong values\n",     \
                   __FILE__, __LINE__, (int_least64_t)(ts_sec),                         \
                   (int_least64_t)(ts_nsec));                                           \
            printf("    Expected: {%ld, %ld}\n", (int_least64_t)(expect_sec),           \
                   (int_least64_t)(expect_nsec));                                       \
            printf("    Got:      {%ld, %ld}\n", (int_least64_t)(got.tv_sec),           \
                   (int_least64_t)(got.tv_nsec));                                       \
            ++result;                                                                   \
        }                                                                               \
    }
    ...
    {                                                                                   \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {                   \
            printf(#func "({%ld, %ld}, {%ld, %ld}) returned wrong values\n",            \
                   (int_least64_t)(ts1_sec), (int_least64_t)(ts1_nsec),                 \
                   (int_least64_t)(ts2_sec), (int_least64_t)(ts2_nsec));                \
            printf("    Expected: {%ld, %ld}\n", (int_least64_t)(expect_sec),           \
                   (int_least64_t)(expect_nsec));                                       \
            printf("    Got:      {%ld, %ld}\n", (int_least64_t)(got.tv_sec),           \
                   (int_least64_t)(got.tv_nsec));                                       \
...
#define TEST_TRINOP(func, ts1_sec, ts1_nsec, ts2_sec, ts2_nsec, ts3_sec, ts3_nsec,      \
                    expect_sec, expect_nsec)                                            \
    {                                                                                   \
        struct timespec ts1 = {.tv_sec = ts1_sec, .tv_nsec = ts1_nsec};                 \
        struct timespec ts2 = {.tv_sec = ts2_sec, .tv_nsec = ts2_nsec};                 \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {                   \
            printf(#func                                                                \
                   ...
                   (int_least64_t)(ts1_sec), (int_least64_t)(ts1_nsec),                 \
                   (int_least64_t)(ts2_sec), (int_least64_t)(ts2_nsec),                 \
                   (int_least64_t)(ts3_sec), (int_least64_t)(ts3_nsec));                \
            printf("    Expected: {%ld, %ld}\n", (int_least64_t)(expect_sec),           \
                   (int_least64_t)(expect_nsec));                                       \
            printf("    Got:      {%ld, %ld}\n", (int_least64_t)(got.tv_sec),           \
                   (int_least64_t)(got.tv_nsec));                                       \
    ...
    {                                                                                   \
        ...
        if (got != expect) {                                                            \
            printf("%s:%d: " #func                                                      \
                   ...
                   __FILE__, __LINE__, (int_least64_t)(ts1_sec),                        \
                   (int_least64_t)(ts1_nsec), (int_least64_t)(ts2_sec),                 \
                   (int_least64_t)(ts2_nsec), got, #expect);                            \
            ++result;                                                                   \
        }                                                                               \
    }
    ...
    {                                                                                   \
        ...
        if (got.tv_sec != expect_sec || got.tv_nsec != expect_nsec) {                   \
            printf("%s:%d: timespec_from_timeval({%ld, %ld}) returned wrong values\n",  \
                   __FILE__, __LINE__, (int_least64_t)(in_sec),                         \
                   (int_least64_t)(in_usec));                                           \
            printf("    Expected: {%ld, %ld}\n", (int_least64_t)(expect_sec),           \
                   (int_least64_t)(expect_nsec));                                       \
            printf("    Got:      {%ld, %ld}\n", (int_least64_t)(got.tv_sec),           \
                   (int_least64_t)(got.tv_nsec));                                       \
    ...
    {                                                                                   \
        ...
        if (got.tv_sec != expect_sec || got.tv_usec != expect_usec) {                   \
            printf("%s:%d: timespec_to_timeval({%ld, %ld}) returned wrong values\n",    \
                   ...
                   (int_least64_t)(ts_nsec));                                           \
            printf("    Expected: {%ld, %ld}\n", (int_least64_t)(expect_sec),           \
                   (int_least64_t)(expect_usec));                                       \
            printf("    Got:      {%ld, %ld}\n", (int_least64_t)(got.tv_sec),           \
                   (int_least64_t)(got.tv_usec));                                       \
            ++result;                                                                   \
        }                                                                               \
    }
...
#define TEST_TO_MS(ts_sec, ts_nsec, expect)                                             \
    {                                                                                   \
        struct timespec ts = {.tv_sec = ts_sec, .tv_nsec = ts_nsec};                    \
        int_least64_t got = timespec_to_ms(ts);                                         \
        if (got != expect) {                                                            \
            printf("%s:%d: timespec_to_ms({%ld, %ld}) returned wrong value\n",          \
                   __FILE__, __LINE__, (int_least64_t)(ts_sec),                         \
                   (int_least64_t)(ts_nsec));                                           \
            printf("    Expected: %ld\n", (int_least64_t)(expect));                     \
            printf("    Got:      %ld\n", got);                                         \
            ++result;                                                                   \
        }                                                                               \
    }
...
int main()
{
    ...
    TEST_BINOP(timespec_mod, INT_LEAST64_MAX, 0, 0, 1, 0, 0);
    TEST_BINOP(timespec_mod, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 1, INT_LEAST64_MAX, 0);
    ...
    TEST_BINOP(timespec_min, INT_LEAST64_MIN, 0, 0, 1, INT_LEAST64_MIN, 0);
    TEST_BINOP(timespec_min, INT_LEAST64_MIN, 0, 0, -1, INT_LEAST64_MIN, 0);
    TEST_BINOP(timespec_min, INT_LEAST64_MIN, 0, INT_LEAST64_MAX, 0, INT_LEAST64_MIN, 0);
    TEST_BINOP(timespec_min, INT_LEAST64_MIN, 0, INT_LEAST64_MIN, 0, INT_LEAST64_MIN, 0);
    TEST_BINOP(timespec_min, INT_LEAST64_MAX, 0, 0, 1, 0, 1);
    TEST_BINOP(timespec_min, INT_LEAST64_MAX, 0, 0, -1, 0, -1);
    TEST_BINOP(timespec_min, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 0);
    TEST_BINOP(timespec_min, INT_LEAST64_MAX, 0, INT_LEAST64_MIN, 0, INT_LEAST64_MIN, 0);
    ...
    TEST_BINOP(timespec_max, INT_LEAST64_MIN, 0, 0, 1, 0, 1);
    TEST_BINOP(timespec_max, INT_LEAST64_MIN, 0, 0, -1, 0, -1);
    TEST_BINOP(timespec_max, INT_LEAST64_MIN, 0, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 0);
    TEST_BINOP(timespec_max, INT_LEAST64_MIN, 0, INT_LEAST64_MIN, 0, INT_LEAST64_MIN, 0);
    TEST_BINOP(timespec_max, INT_LEAST64_MAX, 0, 0, 1, INT_LEAST64_MAX, 0);
    TEST_BINOP(timespec_max, INT_LEAST64_MAX, 0, 0, -1, INT_LEAST64_MAX, 0);
    TEST_BINOP(timespec_max, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 0, INT_LEAST64_MAX, 0);
    TEST_BINOP(timespec_max, INT_LEAST64_MAX, 0, INT_LEAST64_MIN, 0, INT_LEAST64_MAX, 0);
